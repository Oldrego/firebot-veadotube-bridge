...
export const peekStateEffect: EffectType<Model> = {
  definition: {
    id: "veadotube:peek-state",
    name: "Veadotube: Peek State",
    description: "Gets the topmost state of a state events node in veadotube",
    ...
    outputs: [
      {
        label: "State",
        description: "The current topmost state of the node",
        defaultName: "state"
      }
    ]
  },
  ...
  onTriggerEvent: async ({ effect }) => {
    ...
    try {
      var response = await instance.peekState(effect.nodeId);
    }
    catch (error) {
