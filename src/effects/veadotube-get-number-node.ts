...
export const getNumberEffect: EffectType<Model> = {
  definition: {
    id: "veadotube:get-number",
    name: "Veadotube: Get Number",
    description: "Get a number node's value in veadotube",
    icon: "fas fa-hashtag",
  ...
  onTriggerEvent: async ({ effect }) => {
    ...
    try {
      var response = await instance.getNumber(effect.nodeId);
    }
    catch (error) {
